  ...
  if (!actualEvent.isCustomEvent)
    return (
      <div style={{ fontSize: '0.6rem', padding: 1, color: 'white' }}>
        <div
          style={{ fontSize: '0.6rem', fontWeight: 500 }}>{startString.substring(0, startString.length - 2) + ' - ' + endString.substring(0, endString.length - 2)}</div>
            ...
            {this.props.classEventsInCalendar.map((event) => {
              if (event.start.getDay() === 1) {
                const style = {
                  ...
                  left: `${((event.start.getHours() - 6) / .16) + ((event.start.getMinutes() / 60) * 6.25)}%`,
                  height: '90%',
                  position: 'absolute',
                  borderRadius: 4,
