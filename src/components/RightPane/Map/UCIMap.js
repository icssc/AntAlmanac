...
import RightPaneStore from '../RightPaneStore.js';
...
export default class UCIMap extends PureComponent {
    ...
    focusOnBuilding = ({ lat, lng, name, acronym, imgURL } = {}) => {
        // Since the condition to display the event information popup is whether the name is defined,
        // an empty name can't activate it
        if (name === undefined) {
            console.error('Tried to focus on building without name and acronym');
            console.log({ lat, lng, name, acronym, imgURL });
        }
        ...
        if (lat === undefined || lng === undefined || imgURL === undefined) {
            ...
            if (imgURL === undefined) {
                const imgURLs = locationData.imageURLs;
                imgURL = imgURLs.length > 0 ? imgURLs[0] : null;
            }
        }
        // Throw error if lat/lng search fails
        if (lat === null || lng === null) console.error('Could not find building: ' + name);

        this.setState({
            lat: lat,
            lng: lng,
            selected: name,
            selected_acronym: acronym !== undefined ? acronym : name.split(' ')[0],
            selected_img: imgURL,
            zoom: 18,
        });
    };
    ...
    componentDidMount = () => {
        ...
        RightPaneStore.on('selectBuilding', this.focusOnBuilding);
        // Inform RightPaneRoot that the map's loaded and ready to focus on a building
    ...
    componentWillUnmount = () => {
        ...
        RightPaneStore.removeListener('selectBuilding', this.focusOnBuilding);
    };
    ...
    handleSearch = (event, searchValue) => {
        if (searchValue) {
            ...
            this.focusOnBuilding({
                lat: searchValue.lat,
                lng: searchValue.lng,
                name: searchValue.name,
                acronym: acronym,
                imgURL: searchValue.imageURLs.length !== 0 ? searchValue.imageURLs[0] : null,
            });
        } else {
