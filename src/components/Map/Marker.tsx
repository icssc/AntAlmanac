import { useRef, useState, useEffect } from 'react'
import Leaflet from 'leaflet';
import { Marker, Popup } from 'react-leaflet';
import { Box, Button, Link, Typography } from '@mui/material';
import { DirectionsWalk as DirectionsWalkIcon } from '@mui/icons-material';

const GOOGLE_MAPS_URL = 'https://www.google.com/maps/dir/?api=1&travelmode=walking&destination=';
const IMAGE_CMS_URL = 'https://cms.concept3d.com/map/lib/image-cache/i.php?mapId=463&image=';

function getMarkerIcon(stackIndex: number, ref?: any) {
  return Leaflet.divIcon({
    /**
     * Adds offset for marker for stacking markers
     */
    iconAnchor: [0, 14 + 16 * stackIndex],

    /** Adds offset for popup for stacking markers
     */
    popupAnchor: [0, -21 - 16 * stackIndex],

    /**
     * removes styling added by leaflet classes
     */
    className: '',

    /**
     * what the marker will look like
     */
    html: ref
  });
}

interface Props {
  markerColor: string;
  image: string;
  location: string;
  lat: number;
  lng: number;
  acronym: string;
  stackIndex: number;
  children?: React.ReactNode;
}

export default function MapMarker(props: Props) {
  const ref = useRef<HTMLDivElement | null>(null);
  const [, setE] = useState<HTMLDivElement | null>(null);

  useEffect(() => {
    if (ref.current) {
      setE(ref.current);
    }
  }, [ref])

  return (
    <>
      <Marker
        position={[props.lat, props.lng]}
        icon={getMarkerIcon(props.stackIndex, ref.current)}
        zIndexOffset={props.stackIndex}
      >
        <Popup>
          {props.location ? (
            <Link
              href={`http://www.classrooms.uci.edu/classrooms/${props.acronym}`}
              target="_blank"
              rel="noopener noreferrer"
            >
              {props.location}
            </Link>
          ) : (
            <Typography>{props.location}</Typography>
          )}

          {props.image && (
            <img src={`${IMAGE_CMS_URL}${props.image}`} alt="Building Snapshot" style={{ width: '100%' }} />
          )}

          {props.children}

          <Button
            variant="contained"
            color="inherit"
            startIcon={<DirectionsWalkIcon />}
            href={`${GOOGLE_MAPS_URL}${props.lat},${props.lng}`}
            target="_blank"
          >
            Directions
          </Button>
        </Popup>
      </Marker>

      <Box sx={{ position: 'relative' }} ref={ref}>
        <Box
          sx={{
            position: 'absolute',
            top: '-1rem',
            width: '1.75rem',
            height: '1.75rem',
            left: '-1rem',
            border: '1px solid #FFFFFF',
            borderRadius: '1.9rem 1.9rem 0',
            backgroundColor: props.markerColor,
            transform: 'rotate(45deg)',
          }}
        ></Box>
        <Box
          sx={{
            position: 'absolute',
            width: '1.75rem',
            height: '1.75rem',
            left: '-1rem',
            top: '-0.75rem',
            color: 'white',
            textAlign: 'center',
          }}
        ></Box>
      </Box>
    </>
  );
}
