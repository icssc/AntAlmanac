...
/**
 * regex parse a meeting time string 
 * @returns the parsed + transformed info
 */
function parseMeetingTime(meetingTime: string) {
  const [, date, , , startHrStr, startMinStr, endHrStr, endMinStr, ampm] = meetingTime.match(
  ...
  return { date, startHr, startMin, endHr, endMin }
}
...
export function getCourseCalendarEvents(courses: Course[]): CourseCalendarEvent[] {
  const calendarEventsForAllCourses = courses.map((course) => {
    const calendarEventsForCourse = course.section.meetings
      ...
      .map((meeting) => {
        const { startHr, startMin, endHr, endMin } = parseMeetingTime(meeting.time)
...
export function getFinalsCalendarEvents(courses: Course[]): CourseCalendarEvent[] {
  const finalsForAllCourses = courses
    ...
    .map((course) => {
      const { date, startHr, startMin, endHr, endMin } = parseMeetingTime(course.section.finalExam)
...
export function getCustomCalendarEvents(customEvents: RepeatingCustomEvent[]): CustomCalendarEvent[] {
  const allCustomCalendarEvents = customEvents.map((customEvent) => {
    const calendarEventsSingleCustom = customEvent.days
      .map((day, dayIndex) => {
        if (day) {
