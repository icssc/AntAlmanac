'use client';
import helpImage from '$planner/asset/zot4plan-import-help.png';

import './ImportZot4PlanPopup.scss';
import { expandAllPlanners, makeUniquePlanName } from '$planner/helpers/planner';
import { addPlanner } from '$planner/helpers/roadmapEdits';
import { markTransfersAsUnread } from '$planner/helpers/transferCredits';
import { useIsLoggedIn } from '$planner/hooks/isLoggedIn';
import { useReviseAndSaveRoadmap } from '$planner/hooks/planner';
...
import { setUserAPExams } from '$planner/store/slices/transferCreditsSlice';
...
const ImportZot4PlanPopup: FC = () => {
    ...
    const obtainImportedRoadmap = async (schedName: string, currYear: string) => {
        ...
        try {
            ...
            // Unknown (undefined) course names will crash the Planner if loaded, so remove them
            let problemCount = 0;
            ...
            if (problemCount > 0) {
                dispatch(
                    setToastMsg(
                        'Partially imported "' + schedName + '" (removed ' + problemCount + ' unknown course(s)'
                    )
                );
                dispatch(setToastSeverity('error'));
                dispatch(setShowToast(true));
            ...
            const revision = addPlanner(
                nextPlanTempId,
                expandedPlanners[0].name,
                expandedPlanners[0].content.yearPlans
            );
                    ...
                    <Box component="form" noValidate>
                        <FormControl>
                            <p>
                                ...
                                classes into a new roadmap, enter the exact name that you used to save your Zot4Plan
