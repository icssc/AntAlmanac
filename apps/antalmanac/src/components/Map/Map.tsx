...
export function getCoursesPerBuilding() {
    ...
    const courseBuildings = courseEvents.flatMap((event) =>
        event.locations.map((location) => location.building),
    );
    ...
    validBuildingCodes.forEach((buildingCode) => {
        coursesPerBuilding[buildingCode] = courseEvents
            .filter((event) =>
                event.locations.map((location) => location.building).includes(buildingCode),
            )
            .map((event) => {
...
export function getCustomEventPerBuilding() {
    ...
    for (let i = 0; i < customEventBuildings.length; i++) {
        customEventBuildings[i] =
            Object.keys(locationIds).find(
                (key) => locationIds[key] === parseInt(customEventBuildings[i]),
            ) || "";
    }
    ...
    const customEventPerBuilding: Record<
        string,
        (localCustomEventType & Building & MarkerContent)[]
    > = {};
        ...
        customEventPerBuilding[validBuildingCodes[i]] = customEvents
            .filter((event) => {
                return (
                    Object.keys(locationIds).find(
                        (key) =>
                            locationIds[key] === parseInt(event.building ? event.building : ""),
...
export default function CourseMap() {
    ...
    useEffect(() => {
        ...
        const building =
            locationID in buildingCatalogue ? buildingCatalogue[locationID] : undefined;
    ...
    const days = useMemo(() => {
        const hasWeekendEvent = calendarEvents.some((event) =>
            weekendIndices.includes(event.start.getDay()),
        );
    ...
    const markersToDisplay = useMemo(() => {
        ...
        const markersToday =
            today === "All"
                ? markerValues
                : markerValues.filter((course) => course.start.toString().includes(today));

        return markersToday
            .sort((a, b) => a.start.getTime() - b.start.getTime())
            .filter(
                (marker, i, arr) =>
                    arr.findIndex((other) => other.sectionCode === marker.sectionCode) === i,
            );
    }, [markers, today]);
            ...
            >
                ...
                {today !== "All" &&
                    startDestPairs.map((startDestPair) => {
                        const latLngTuples = startDestPair.map(
                            (marker) => [marker.lat, marker.lng] as LatLngTuple,
                        );
                            ...
                            >
                                <Box>
                                    <Typography variant="body1">
                                        ...
                                        {marker.title} {marker.sectionType}
                                    </Typography>
                                    <Typography variant="body1">
                                        <span style={{ fontWeight: "bold" }}>
                                            Room{allRoomsInBuilding.length > 1 && "s"}:
                                        </span>{" "}
                                        {marker.locations[0].building}{" "}
                ...
                {customEventMarkersToDisplay.map((customEventMarkers, index) => {
                    const customEventSameBuildingPrior = customEventMarkersToDisplay.slice(
                        0,
                        index,
                    );
