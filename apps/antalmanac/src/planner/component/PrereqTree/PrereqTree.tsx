...
import type { Prerequisite, PrerequisiteNode, PrerequisiteTree } from '@packages/planner-types';
import Link from 'next/link';
import { type FC } from 'react';

import { createTooltipOffset } from '../../helpers/slotProps';
import { type CourseGQLData, type CourseLookup } from '../../types/types';
                ...
                <Box>
                    {!props.label.startsWith('AP ') ? (
                        <Link
                            target="_blank"
                            href={
                                '/planner/course/' + encodeURIComponent(props.label.split('(')[0].replace(/\s+/g, ''))
                            }
                            role="button"
                            className="node"
                        >
