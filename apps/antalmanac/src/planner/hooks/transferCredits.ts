...
import {
    ...
    type TransferredCourse,
    type TransferredGE,
    type TransferredUncategorized,
} from '@packages/planner-types';
import { type components } from '@packages/planner-types/src/generated/anteater-api-types';
...
export function useLoadTransferredCredits() {
    ...
    useEffect(() => {
        ...
        // Load in parallel
        Promise.all([loadAPInfo, loadCourses, loadAPs, loadGEs, loadOther]).then(() =>
            dispatch(setDataLoadState('done'))
        );
