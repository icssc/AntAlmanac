...
import {
    type QuarterName,
    type SavedPlannerData,
    type SavedPlannerQuarterData,
    type SavedRoadmap,
    type TransferredAPExam,
...
import { getAPIApExams, tryMatchAp } from '../helpers/transferCredits';
...
/**
 * Convert a Zot4Plan course name into a Planner course ID
 */
const convertIntoCourseID = (zot4PlanCourse: string): string => {
    // Planner course IDs are the same as Zot4Plan course IDs except all spaces are removed
...
/**
 * Fetches all AP exams from the Zot4Plan schedule, matching their names to Planner AP Exam names; filter out duplicates
 */
...
const zot4PlanImportRouter = router({
    /**
     * Get a roadmap formatted for the Planner based on a Zot4Plan schedule by name
