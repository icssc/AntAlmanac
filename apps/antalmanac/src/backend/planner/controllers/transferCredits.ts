...
import { type APExam } from '@packages/planner-types';
...
const transferCreditsRouter = router({
    ...
    updateTransferredCourse: userProcedure.input(zodTransferredCourse).mutation(async ({ ctx, input }) => {
        await db
            ...
            .where(
                and(
                    eq(transferredCourse.userId, ctx.session.userId!),
                    eq(transferredCourse.courseName, input.courseName)
                )
            );
    }),
    ...
    getSavedAPExams: userProcedure.query(async ({ ctx }): Promise<TransferredAPExam[]> => {
        ...
        const res = await db
            .select({
                examName: transferredApExam.examName,
                score: transferredApExam.score,
                units: transferredApExam.units,
            })
    ...
    updateUncategorizedCourse: userProcedure.input(zodTransferredUncategorized).mutation(async ({ ctx, input }) => {
        await db
            ...
            .set({ units: input.units })
            .where(
                and(eq(transferredMisc.userId, ctx.session.userId!), eq(transferredMisc.courseName, input.name ?? ''))
            );
    }),
