...
export class SchedulesRDS {
    ...
    static async upsertUserData(
        ...
        return db.transaction(async (tx) => {
            const scheduleIdMap = await SchedulesRDS.upsertSchedulesAndContents(tx, userId, saveState.schedules);
    ...
    private static async upsertSchedulesAndContents(
        ...
        await Promise.all(
            prepared.flatMap(({ schedule, dbId }) => [
                SchedulesRDS.upsertCourses(tx, dbId, schedule.courses).catch((error) => {
                    throw new Error(`Failed to upsert courses for ${schedule.scheduleName}: ${error}`);
                }),
                SchedulesRDS.upsertCustomEvents(tx, dbId, schedule.customEvents).catch((error) => {
    ...
    static async getScheduleById(
        ...
        return db.transaction(async (tx) => {
            ...
            const scheduleArray = UsersRDS.aggregateUserData(sectionResults, customEventResults);
